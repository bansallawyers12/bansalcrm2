					...
					<div class="card">
						<div class="card-header">
							...
							<div class="card-header-action">
								<a href="<?php echo e(route('agent.clients.index')); ?>" class="btn btn-primary">Client List</a>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="card-body">
							...
							<span class="author-avtar" style="background: rgb(68, 182, 174);"><b><?php echo e(substr($fetchedData->first_name, 0, 1)); ?><?php echo e(substr($fetchedData->last_name, 0, 1)); ?></b></span>
								...
								<div class="author-mail_sms">
									...
									<a href="<?php echo e(URL::to('/agent/clients/edit/'.base64_encode(convert_uuencode(@$fetchedData->id)))); ?>" title="Edit"><i class="fa fa-edit"></i></a>
					...
					<div class="card">
						...
						<div class="card-body">
								...
								foreach($rs as $r){
									...
									?>
										<span class="ui label ag-flex ag-align-center ag-space-between" style="display: inline-flex;">
											<span class="col-hr-1" style="font-size: 12px;"><?php echo e(@$stagd->name); ?></span> 
										</span>
									<?php
									}
								}								
							...
							<p class="clearfix"> 
								...
								<span class="float-right text-muted"><?php echo e($fetchedData->client_id); ?></span>
							</p>
							<p class="clearfix"> 
								<span class="float-left">Internal Id:</span>
								<span class="float-right text-muted"><?php echo e($fetchedData->id); ?></span>
							</p>
							<p class="clearfix"> 
								<span class="float-left">Date Of Birth:</span>
								<span class="float-right text-muted"><?php echo e($fetchedData->dob); ?></span>
							</p>
							<p class="clearfix"> 
								<span class="float-left">Phone No:</span>
								<span class="float-right text-muted"><?php echo e($fetchedData->phone); ?></span>
							</p>
							<p class="clearfix"> 
								<span class="float-left">Email:</span>
								<span class="float-right text-muted"><?php echo e($fetchedData->email); ?></span>
							</p>
							<p class="clearfix"> 
							...
							<p class="clearfix"> 
								...
								<span class="float-right text-muted"><?php echo e($fetchedData->country_passport); ?></span>
							</p>
							<p class="clearfix"> 
								<span class="float-left">Passport Number:</span>
								<span class="float-right text-muted"><?php echo e($fetchedData->passport_number); ?></span>
							</p>
							<p class="clearfix"> 
							...
							<p class="clearfix"> 
								...
								<span class="float-right text-muted"><?php echo e($fetchedData->visaExpiry); ?></span>
							</p>
							<p class="clearfix"> 
								<span class="float-left">Visa type:</span>
								<span class="float-right text-muted"><?php echo e($fetchedData->visa_type); ?></span>
							</p> 
							<?php
								// PostgreSQL doesn't accept empty strings for integer columns - check before querying
								$addedby = null;
							...
							<div class="client_assign client_info_tags"> 
								...
								<div class="client_info">
									...
									<div class="cl_name">
										...
										<span class="email"><?php echo e(@$assignee->email); ?></span>
									</div>
								</div>
								<?php else: ?>
									-
							...
							<div class="client_assign client_info_tags"> 
								...
								<div class="client_info">
									...
									?>
									    <li><a target="_blank" href="<?php echo e(URL::to('/agent/clients/detail/'.base64_encode(convert_uuencode(@$res->id)))); ?>"><?php echo e($res->first_name); ?> <?php echo e($res->last_name); ?></a></li>
									<?php } ?>
									<?php
									...
									if(!empty($exploder)) {
										foreach($exploder AS $EXP){ 
											...
											if(!empty(trim($EXP)) && trim($EXP) !== '') {
												...
												if($relatedclients) {
									?>
													<li><a target="_blank" href="<?php echo e(URL::to('/agent/clients/detail/'.base64_encode(convert_uuencode(@$relatedclients->id)))); ?>"><?php echo e($relatedclients->first_name); ?> <?php echo e($relatedclients->last_name); ?></a></li>
									<?php 
												}
											}
										} 
										...
										<table class="table text_wrap table-2">
											...
											foreach(\App\Models\Application::where('client_id', $fetchedData->id)->orderby('created_at','Desc')->get() as $alist){
												...
												<tr id="id_<?php echo e($alist->id); ?>">
													<td><a class="openapplicationdetail" data-id="<?php echo e($alist->id); ?>" href="javascript:;" style="display:block;"><?php echo e(@$productdetail->name); ?></a> <small><?php echo e(@$partnerdetail->partner_name); ?> (<?php echo e(@$PartnerBranch->name); ?>)</small></td> 
													...
													<td><?php echo e(@$alist->sale_forcast); ?></td>
													<td><?php echo e(date('Y-m-d', strtotime(@$alist->created_at))); ?></td> 
													<td><?php echo e(date('Y-m-d', strtotime(@$alist->updated_at))); ?></td> 
													<td>
											...
											<div class="extra_content">
												...
												<div class="left">
													...
													<div class="note_modify">
														<small>Last Modified <span><?php echo e(date('Y-m-d', strtotime($list->updated_at))); ?></span></small>
													</div>
												</div>  
												<div class="right">
													<div class="dropdown d-inline dropdown_ellipsis_icon">
...
<?php echo $__env->make('Agent/clients/addclientmodal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>  
<?php echo $__env->make('Agent/clients/editclientmodal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>   
...
$(document).delegate('#confirmpublishdocModal .acceptpublishdoc', 'click', function(){
	$('.popuploader').show(); 
	$.ajax({
		url: '<?php echo e(URL::to('/agent/')); ?>/'+'application/publishdoc',
		type:'GET',
		datatype:'json',
	...
	$(document).delegate('#confirmModal .accept', 'click', function(){
		...
		$('.popuploader').show(); 
		$.ajax({
			url: '<?php echo e(URL::to('/agent/')); ?>/'+delhref,
			type:'GET',
			datatype:'json',
			data:{note_id:notid},
			success:function(response){
	...
	$(document).delegate('.pinnote', 'click', function(){
		$('.popuploader').show(); 
		$.ajax({
			url: '<?php echo e(URL::to('/agent/pinnote')); ?>/',
			type:'GET',
			datatype:'json',
	...
	$('.js-data-example-ajaxcc').select2({
		...
		dropdownParent: $('#create_note'),
		  ajax: {
			url: '<?php echo e(URL::to('/agent/clients/get-recipients')); ?>',
			dataType: 'json',
  ...
  return repo.name || repo.text;
	...
	$(document).delegate('.opennoteform', 'click', function(){
		...
		$('#create_note input[name="noteid"]').val(v);
			$('.popuploader').show(); 
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getnotedetail')); ?>',
			type:'GET',
			datatype:'json',
			data:{note_id:v},
			success:function(response){
	...
	$(document).delegate('.viewnote', 'click', function(){
		...
		$('#view_note input[name="noteid"]').val(v);
			$('.popuploader').show(); 
		$.ajax({
			url: '<?php echo e(URL::to('/agent/viewnotedetail')); ?>',
			type:'GET',
			datatype:'json',
			data:{note_id:v},
			success:function(response){
	...
	$(document).delegate('.viewapplicationnote', 'click', function(){
		...
		$('#view_application_note input[name="noteid"]').val(v);
			$('.popuploader').show(); 
		$.ajax({
			url: '<?php echo e(URL::to('/agent/viewapplicationnote')); ?>',
			type:'GET',
			datatype:'json',
			data:{note_id:v},
			success:function(response){
	...
	$(document).delegate('.add_appliation #workflow', 'change', function(){
				...
				if(v != ''){
						$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getpartnerbranch')); ?>',
			type:'GET',
			data:{cat_id:v},
			success:function(response){
				$('.popuploader').hide();
	...
	$(document).delegate('.add_appliation #partner','change', function(){
				...
				if(v != ''){
					$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getbranchproduct')); ?>',
			type:'GET',
...
$(document).delegate('.change_client_status', 'click', function(e){
	...
	$(this).addClass('active');
	 ...
	 $.ajax({
		url: '<?php echo e(URL::to('/agent/change-client-status')); ?>',
		type:'GET',
		datatype:'json',
...
$(document).delegate('.selecttemplate', 'change', function(){
	var v = $(this).val();
	$.ajax({
		url: '<?php echo e(URL::to('/agent/get-templates')); ?>',
		type:'GET',
		datatype:'json',
		data:{id:v},
		success: function(response){
...
$(document).delegate('#intrested_workflow', 'change', function(){
				...
				if(v != ''){
						$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getpartner')); ?>',
			type:'GET',
			data:{cat_id:v},
			success:function(response){
				$('.popuploader').hide();
	...
	$(document).delegate('#intrested_partner','change', function(){
				...
				if(v != ''){
					$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getproduct')); ?>',
			type:'GET',
			data:{cat_id:v},
			success:function(response){
				$('.popuploader').hide();
	...
	$(document).delegate('#intrested_product','change', function(){
				...
				if(v != ''){
					$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getbranch')); ?>',
			type:'GET',
			data:{cat_id:v},
			success:function(response){
				$('.popuploader').hide();
...
$('.custom-error-msg').html('<span class="alert alert-danger">'+ress.message+'</span>');	
	...
	$(document).delegate('.converttoapplication','click', function(){
				...
				if(v != ''){
					$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/convertapplication')); ?>',
			type:'GET',
...
$(document).on('click', '#application-tab', function () {
	...
	$(document).delegate('.documnetlist .drow .btn-primary', 'click', function () {
			...
			$.ajax({
			   ...
			   url: '<?php echo e(URL::to('/agent/renamedoc')); ?>',
...
$(document).delegate('.opencreate_task', 'click', function () {
	...
	$(document).delegate('#confirmEducationModal .accepteducation', 'click', function(){
		...
		$('.popuploader').show(); 
		$.ajax({
			url: '<?php echo e(URL::to('/agent/')); ?>/delete-education',
			type:'GET',
			datatype:'json',
			data:{edu_id:eduid},
			success:function(response){
    ...
    $(document).delegate('#educationform #subjectlist', 'change', function(){
				...
				if(v != ''){
						$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getsubjects')); ?>',
			type:'GET',
			data:{cat_id:v},
			success:function(response){
				$('.popuploader').hide();
	...
	$(document).delegate('.edit_appointment', 'click', function(){
		...
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getAppointmentdetail')); ?>',
			type:'GET',
			data:{id:v},
			success:function(response){
				$('.popuploader').hide();
   ...
   $(".invitesselects2").select2({
	...
	$(document).delegate('.editeducation', 'click', function(){
		...
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getEducationdetail')); ?>',
			type:'GET',
			data:{id:v},
			success:function(response){
				$('.popuploader').hide();
	...
	$(document).delegate('.interest_service_view', 'click', function(){
		...
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getintrestedservice')); ?>',
			type:'GET',
			data:{id:v},
			success:function(response){
				$('.popuploader').hide();
	...
	$(document).delegate('.openeditservices', 'click', function(){
		...
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getintrestedserviceedit')); ?>',
			type:'GET',
			data:{id:v},
			success:function(response){
				$('.popuploader').hide();
...
$(document).delegate('.paymentAmount','keyup', function(){
	...
	if(isset($_GET['tab']) && $_GET['tab'] == 'application' && isset($_GET['appid']) && $_GET['appid'] != ''){
		...
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getapplicationdetail')); ?>',
			type:'GET',
			data:{id:appliid},
			success:function(response){
				$('.popuploader').hide();
				...
				locale: { format: "YYYY-MM-DD",cancelLabel: 'Clear' },
					...
					$('#popuploader').show();
					$.ajax({
						url:"<?php echo e(URL::to('/agent/application/updateintake')); ?>",
						method: "GET", // or POST
						dataType: "json",
				...
				locale: { format: "YYYY-MM-DD",cancelLabel: 'Clear' },
					...
					$('#popuploader').show();
					$.ajax({
						url:"<?php echo e(URL::to('/agent/application/updateexpectwin')); ?>",
						method: "GET", // or POST
						dataType: "json",
						data: {from: start.format('YYYY-MM-DD'), appid: appliid},
						success:function(result) {
				...
				locale: { format: "YYYY-MM-DD",cancelLabel: 'Clear' },
					...
					$('#popuploader').show();
					$.ajax({
						url:"<?php echo e(URL::to('/agent/application/updatedates')); ?>",
						method: "GET", // or POST
						dataType: "json",
...
$(document).delegate('.openpaymentschedule', 'click', function(){
	...
	$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/addscheduleinvoicedetail')); ?>',
			type: 'GET',
...
$(document).delegate('.nextstage', 'click', function(){
	...
	$('.popuploader').show();
	$.ajax({
		url: '<?php echo e(URL::to('/agent/updatestage')); ?>',
		type:'GET',
		datatype:'json',
...
$(document).delegate('.acceptapplication', 'click', function(){
	...
	$('.popuploader').show();
	$.ajax({
		url: '<?php echo e(URL::to('/agent/completestage')); ?>',
		type:'GET',
		datatype:'json',
		data:{id:appliid, client_id:'<?php echo e($fetchedData->id); ?>'},
		success:function(response){
...
$(document).delegate('.backstage', 'click', function(){
	...
	if(stage == 'Application'){
		...
		$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/updatebackstage')); ?>',
			type:'GET',
			datatype:'json',
			data:{id:appliid, client_id:'<?php echo e($fetchedData->id); ?>'},
			success:function(response){
...
$(document).delegate('#notes-tab', 'click', function(){
		...
		$('.ifapplicationdetailnot').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/getapplicationnotes')); ?>',
			type:'GET',
			data:{id:appliid},
			success:function(response){
				$('.popuploader').hide();
  ...
  $('#attachments').on('change',function(){
	...
	$(document).delegate('.openpaymentfee', 'click', function(){
		...
		$.ajax({
			url: '<?php echo e(URL::to('/agent/showproductfee')); ?>',
			type:'GET',
			data:{id:appliid},
			success:function(response){
				$('.popuploader').hide(); 
	...
	$(document).delegate('.openpaymentfeeserv', 'click', function(){
		...
		$.ajax({
			url: '<?php echo e(URL::to('/agent/showproductfeeserv')); ?>',
			type:'GET',
			data:{id:appliid},
			success:function(response){
				$('.popuploader').hide(); 
	...
	$(document).delegate('.editpaymentschedule', 'click', function(){ 
		...
		$('.popuploader').show();
		$.ajax({
			url: '<?php echo e(URL::to('/agent/scheduleinvoicedetail')); ?>',
			type: 'GET',
...
$(document).ready(function() {
        ...
        $('.popuploader').show(); 
          $.ajax({
            url: "<?php echo e(URL::to('/agent/application/checklistupload')); ?>",
            method: "POST",
            data: form_data,
            datatype: 'json',
            contentType: false,
	...
	<div class="modal-dialog">
		<div class="modal-content"> 
			...
			<div class="modal-body">
				<form method="post" action="<?php echo e(URL::to('/agent/application/saleforcast')); ?>" name="saleforcast" id="saleforcast" autocomplete="off" enctype="multipart/form-data">
				<?php echo csrf_field(); ?> 
...
<div class="modal fade custom_modal" id="application_ownership" tabindex="-1" role="dialog" aria-labelledby="applicationModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content"> 
			...
			<div class="modal-body">
				<form method="post" action="<?php echo e(URL::to('/agent/application/application_ownership')); ?>" name="xapplication_ownership" id="xapplication_ownership" autocomplete="off" enctype="multipart/form-data">
				<?php echo csrf_field(); ?> 
...
<div class="modal fade custom_modal" id="superagent_application" tabindex="-1" role="dialog" aria-labelledby="applicationModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content"> 
			...
			<div class="modal-body">
				<form method="post" action="<?php echo e(URL::to('/agent/application/spagent_application')); ?>" name="spagent_application" id="spagent_application" autocomplete="off" enctype="multipart/form-data">
				<?php echo csrf_field(); ?> 
...
<div class="modal fade custom_modal" id="subagent_application" tabindex="-1" role="dialog" aria-labelledby="applicationModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content"> 
			...
			<div class="modal-body">
				<form method="post" action="<?php echo e(URL::to('/agent/application/sbagent_application')); ?>" name="sbagent_application" id="sbagent_application" autocomplete="off" enctype="multipart/form-data">
				<?php echo csrf_field(); ?> 
...
<div class="modal fade custom_modal" id="tags_clients" tabindex="-1" role="dialog" aria-labelledby="applicationModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="post" action="<?php echo e(URL::to('/agent/save_tag')); ?>" name="stags_application" id="stags_application" autocomplete="off" enctype="multipart/form-data">
				<?php echo csrf_field(); ?> 
						...
						<div class="col-12 col-md-12 col-lg-12">
							<div class="form-group">
								...
								?>
									...
									<?php $__currentLoopData = $stagd; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
										<option <?php if(in_array($sa->id, $r)){ echo 'selected'; } ?> value="<?php echo e($sa->id); ?>"><?php echo e($sa->name); ?></option>
									<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
								</select>
...
<div class="modal fade custom_modal" id="application_opensaleforcastservice" tabindex="-1" role="dialog" aria-labelledby="applicationModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content"> 
			...
			<div class="modal-body">
				<form method="post" action="<?php echo e(URL::to('/agent/application/saleforcastservice')); ?>" name="saleforcastservice" id="saleforcastservice" autocomplete="off" enctype="multipart/form-data">
				<?php echo csrf_field(); ?> 
				<input type="hidden" name="fapp_id" id="fapp_id" value="">
					<div class="row">
						<div class="col-4 col-md-4 col-lg-4">
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.agent', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\bansalcrm2\resources\views\Agent\clients\detail.blade.php ENDPATH**/ ?>