 ...
 * Admin Partner Detail - Promotion Handlers
 *
 * Handles promotion creation, status changes, and edit form loading.
 *
 * Dependencies:
 *   - jQuery
 *   - Select2
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                if(obj.status){
                    ...
                    $(".changepromotonstatus[data-id="+appliid+"]").attr('data-status', updated_status);
                    $.ajax({
                        url: App.getUrl('getPromotions') || (App.getUrl('siteUrl') + '/get-promotions'),
                        type:'GET',
                        data:{clientid: PageConfig.partnerId},
                        success: function(responses){
                            $('.promotionlists').html(responses);
                        }
                    });
