...
class SignatureDashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $counts = $this->signatureDashboardCounts($user);

        return view('crm.signatures.dashboard', compact('documents', 'counts', 'user'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('crm.signatures.create', compact('user', 'document'));
    }
    ...
    public function show($id)
    {
        ...
        return view('crm.signatures.show', compact('document'));
    }
     ...
     */
    private function signatureDashboardCounts(Staff $user): array
    {
        $now = now();
        ...
        $stats = Document::query()
            ...
            ->selectRaw('
                COUNT(*) as all_count,
                SUM(CASE WHEN created_by = ? THEN 1 ELSE 0 END) as sent_by_me,
                SUM(CASE WHEN status = ? THEN 1 ELSE 0 END) as all_pending,
                SUM(CASE WHEN status = ? THEN 1 ELSE 0 END) as pending,
                SUM(CASE WHEN status = ? THEN 1 ELSE 0 END) as signed,
                SUM(CASE WHEN due_at IS NOT NULL AND due_at < ? AND status != ? THEN 1 ELSE 0 END) as overdue
            ', [$user->id, 'sent', 'sent', 'signed', $now, 'signed'])
            ->first();

        $all = (int) ($stats->all_count ?? 0);

        return [
            'sent_by_me' => (int) ($stats->sent_by_me ?? 0),
            'visible_to_me' => $all,
            'pending' => (int) ($stats->pending ?? 0),
            'signed' => (int) ($stats->signed ?? 0),
            'overdue' => (int) ($stats->overdue ?? 0),
            'all' => $all,
            'all_pending' => (int) ($stats->all_pending ?? 0),
        ];
    }
