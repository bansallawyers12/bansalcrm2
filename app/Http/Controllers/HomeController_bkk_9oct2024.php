...
class HomeController extends Controller
{
	...
	public function getdatetime(Request $request)
    {
      //dd($request->all());
        ...
        /*
        noe       service          calendar    slots
        1 PR     1 Mig Advice        ajay      paid
        2 TPR    1 Mig Advice        ajay      paid
        3 JRP    1 Mig Advice        ajay      paid
        4 Tour   1 Mig Advice        ajay      paid
        5 Educ   1  Mig Advice       ajay      paid
        6 Comp   1 Mig Advice        ajay      paid
        7 Visa can 1 Mig Advice      ajay      paid
        8 UK/CAN   1 Mig Advice      ajay      paid

        1 PR     2 Mig Consul       ajay        free
        2 TPR    2 Mig Consul       shubam      free
        3 JRP    2 Mig Consul       shubam      free
        4 Tour   2 Mig Consul       toursit     free
        5 Educ   2  Mig Consul      education   free
        6 Comp   2 Mig Consul       ajay        free
        7 Visa Can 2 Mig Consul     ajay        free
        8 UK/CAN   2 Mig Consul     ajay        free
        */
        /*if( $enquiry_item == 1 || $enquiry_item == 6  || $enquiry_item == 7 || $enquiry_item == 8 ) {
            if( $req_service_id  == 1) {
                $final_service_id = 1;
            }  if( $req_service_id  == 2) {
                $final_service_id = 2;
        ...
        if( $enquiry_item == 2 || $enquiry_item == 3 || $enquiry_item == 4 || $enquiry_item == 5) {
            if( $req_service_id  == 1) {
                $final_service_id = 1;
            }  if( $req_service_id  == 2) {
                $final_service_id = 2;
        ...
        $service = \App\BookService::where('id', $request->id)->first();
        //$service = \App\BookService::where('id', $final_service_id)->first();
	   ...
	   if($service){
		   $weekendd  =array();
			...
			if($service->disabledates != ''){
				$dates = json_decode($service->disabledates);
		   ...
		   return json_encode(array('success'=>true, 'duration' =>$service->duration,'weeks' => $weekendd,'start_time' =>$start_time,'end_time'=>$end_time,'disabledatesarray'=>$disabledatesarray));
	   }else{
	...
	public function getdisableddatetime(Request $request)
    {
        ...
        $enquiry_item = $requestData['enquiry_item'];
        $req_service_id = $requestData['service_id'];
        //echo $enquiry_item."===".$req_service_id; die;
        /*noe  service   calendar   slots
        1     1         ajay        paid
        1     2         ajay        paid
      ...
      if( $request->enquiry_item == 1 || $request->enquiry_item == 6 || $request->enquiry_item == 7 || $request->enquiry_item == 8
            ...
            $service = \App\Appointment::whereDate('date', $datey)
            ->where(function ($query) {
                $query->whereIn('noe_id', [1,6,7,8])
                    ->orWhere('service_id', 1);
            })->exists();

            $servicelist = \App\Appointment::whereDate('date', $datey)
                ->where(function ($query) {
                    $query->whereIn('noe_id', [1,6,7,8])
                        ->orWhere('service_id', 1);
                })->get();
            ...
            $service = \App\Appointment::whereIn('noe_id', [2,3])->whereIn('service_id', [2])->whereDate('date', $datey)->exists();
            $servicelist = \App\Appointment::whereIn('noe_id', [2,3])->whereIn('service_id', [2])->whereDate('date', $datey)->get();
        }
        else if ( $request->enquiry_item == 4 && $request->service_id == 2 )  { //Tourist
            $service = \App\Appointment::where('noe_id', $request->enquiry_item)->where('service_id', $request->service_id)->whereDate('date', $datey)->exists();
            $servicelist = \App\Appointment::where('noe_id', $request->enquiry_item)->where('service_id', $request->service_id)->whereDate('date', $datey)->get();
        }
