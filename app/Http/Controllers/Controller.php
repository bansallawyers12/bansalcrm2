...
class Controller extends BaseController
{
	...
	protected function send_compose_template($content, $sendername, $to = null, $subject = null, $sender = null, $array = array(), $cc = array()) 
	{
        ...
        if( $sender != 'noreply@bansalimmigration.com.au' ){
            $sender = 'noreply@bansalimmigration.com.au';
        }
...
*/ 
			...
			$(document).delegate('.usefile', 'click', function(){
				...
				if(v == 'single'){
					var v = $(this).attr('vcc');
					$('#'+v+' .upload-box').hide();
					$('#'+v+' .dungdt-upload-box').addClass('active');
					
					$('#'+v+' .attach-demo').html('<img src="'+$(this).attr('imgsrc')+'">');
					$('#'+v+' #file_'+v).val($(this).attr('vfileid'));
					$('#'+v+' #<?php echo $fieldname2; ?>').val($(this).attr('vfile_name'));
					$('#cdn-browser-modal').modal('hide');
				}else{
					var v = $(this).attr('vcc');
					//$('#'+v+' .upload-box').hide();
					//$('#'+v+' .dungdt-upload-box').addClass('active');
					var vfileid = $(this).attr('vfile_name');
					var imgs = $(this).attr('imgsrc');
					var splitimg = imgs.split(',');	
					var splitvfileid = vfileid.split(',');	
					var ht = '';
					for(var i =0; i<splitimg.length; i++ ){	
						ht += '<div class="gal_item"><input type="hidden" name="imageids[]" value="'+splitvfileid[i]+'"><img style="width: 100px;" class="img-fluid" src="'+splitimg[i]+'"><a href="javascript:;" class="remove_img"><i class="fa fa-trash"></i></a></div>';
	...
	$(document).delegate('.delete', 'click', function(){
		var v = $(this).attr('cid');
		$('#'+v+' .upload-box').show();
		$('#'+v+' .dungdt-upload-box').removeClass('active');
			$('#'+v+' #file_'+v).val('');
		$('#'+v+' #<?php echo $fieldname2; ?>').val('');
		$('#'+v+' .attach-demo').html('');
	});
	
	$(document).delegate('.remove_img', 'click', function(){
		$(this).parent().remove();
	});
