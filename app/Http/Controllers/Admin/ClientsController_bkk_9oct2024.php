  	...
  	public function addAppointmentBook(Request $request){
		...
		if($saved){
			...
			if(isset($request->type) && $request->atype == 'application'){
                ...
                $objs->description = '<p><span class="text-semi-bold">scheduled an appointment on '.$appoint_date_val_formated.' at '.$request->appoint_time.' for '.$request->description.'</span></p>';
                $objs->subject = $subject;
			    $objs->save();
			}
