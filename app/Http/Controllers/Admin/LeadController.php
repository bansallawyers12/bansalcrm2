...
class LeadController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $assignableStaff = LeadCreateAssignees::assignableStaff();

        return view('Admin.leads.create', compact('assignableStaff'));
    }
    ...
    public function assign(Request $request)
    {
        ...
        if ($admin) {
            ...
            if ($currentAssignee != '' && $currentAssignee != null) {
                ...
                $assignfrom = Staff::find($currentAssignee);
                $assignto = Staff::find($requestData['assignto']);
    ...
    public function store(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            $this->validate($request, [
                ...
                'service' => 'required',
                'assign_to' => ['required', 'integer', Rule::in(LeadCreateAssignees::allowedStaffIds())],
    ...
    protected function createAdminFromRequestData(array $requestData, string $dob, string $visa_expiry_date, string $related_files): ?int
    {
        ...
        if ($firstAssigneeId !== null && $firstAssigneeId !== '' && is_numeric($firstAssigneeId)) {
            $staff = Staff::find((int) $firstAssigneeId);
    ...
    public function is_contactno_unique(Request $request)
    {
        ...
        // Admins table holds both clients and leads - single count
        $phone_count = Admin::where('phone', $contact)->count();
        if ($phone_count > 0) {
            $response['status'] = 1;
