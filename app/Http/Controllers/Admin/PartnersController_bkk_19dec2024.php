...
<?php } ?>
    ...
    public function uploadpartnerfetchmail(Request $request){ //dd($request->all());
        //$request->validate([ 'email_file' => 'required|mimes:msg|max:2048']);
        $request->validate([
            'email_file' => 'required|array', // Ensure the field is an array
            'email_file.*' => 'required|mimes:msg|max:2048', // Validate each file in the array
        ]);
	...
	public function followupstore_partner(Request $request){
        ...
        if(!$saved) {
            ...
            $objs->subject = 'Partner assigned action to '.@$assignee_name;
