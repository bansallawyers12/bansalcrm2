...
class PartnersController extends Controller
{
	...
	public function index(Request $request)
	{
        ...
        ->orderByDesc('student_count') // Order by student_count in descending order
        ->sortable(['id' => 'desc'])
        ->paginate(config('constants.limit')); //dd($lists);
...
<?php } ?>
        ...
        if( $requestData['function_type'] == 'add')
        {
            ...
            if(isset($requestData['invoice_date'])){
                ...
                for($i=0; $i<count($requestData['description']); $i++){
                    //$finalArr[$i]['invoice_date'] = $requestData['invoice_date'];
                    //$finalArr[$i]['invoice_no'] = $requestData['invoice_no'];
                    //$finalArr[$i]['student_id'] = $requestData['student_id'][$i];
                    //$finalArr[$i]['student_dob'] = $requestData['student_dob'][$i];
                    //$finalArr[$i]['student_name'] = $requestData['student_name'][$i];
                    //$finalArr[$i]['student_ref_no'] = $requestData['student_ref_no'][$i];
                    //$finalArr[$i]['course_name'] = $requestData['course_name'][$i];
                    //$finalArr[$i]['student_info_id'] = $requestData['student_info_id'][$i];
    ...
    public function updateStudentStatus(Request $request)
	{
        ...
        if ($updatedRows > 0) {
            ...
            //return response()->json(['message' => 'No changes made or student not found.'], 400);
        }
        echo json_encode($response);
    }
    ...
    public function printpreviewcreateinvoice(Request $request, $id){
        ...
        if(!empty($record_get)){
            ...
            $admin = DB::table('admins')->select('profile_img')->where('id',1)->first();
        }
        set_time_limit(3000);
        $pdf = PDF::setOptions([
    ...
    public function updateInvoiceSentOptionToYes(Request $request)
	{
        ...
        $upd = DB::table('partner_student_invoices')->where('invoice_type', 1)->where('invoice_id', $sel_invoice_id)->update(['sent_option' => 'Yes','sent_date' => $sent_date]);
        //dd($upd);
        if($upd) {
            $response['status'] 	= 	true;
