...
class ProfessorController extends BaseController
{
    ...
    public function professorsList(Request $request)
    {
		...
		{	
			$professorQuery	= Professor::select('id', 'organisation_role', 'which_organisation', 'first_name', 'last_name', 'order_number', 'org_professor_image')->where('status', '=', 1);
		}
		else
		{
			$professorQuery	= Professor::select('id', 'organisation_role', 'which_organisation', 'first_name', 'last_name', 'order_number', 'org_professor_image')->where('status', '=', 1);
		...
		$success['image_base_path'] 	=  asset('img/profile_imgs').'/';
