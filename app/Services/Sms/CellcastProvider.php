...
class CellcastProvider implements SmsProviderInterface
{
    ...
    public function __construct()
    {
        ...
        $this->baseUrl = rtrim(config('services.cellcast.base_url', 'https://cellcast.com.au/api/v3'), '/');
    }

    /**
     * Send SMS via Cellcast API (v3)
     */
        ...
        while ($attempt < $this->maxRetries) {
            try {
                ...
                if ($response->successful()) {
                    ...
                    // Cellcast v3 API: meta.code === 200
                    if (isset($data['meta']['code']) && $data['meta']['code'] === 200) {
                        return [
                            'success' => true,
                            'data' => $data['data'] ?? $data,
                            'message' => $data['msg'] ?? $data['message'] ?? 'SMS sent successfully'
                        ];
                    } else {
                        $errorMessage = $data['msg'] ?? $data['message'] ?? $data['meta']['message'] ?? 'Unknown API error';
