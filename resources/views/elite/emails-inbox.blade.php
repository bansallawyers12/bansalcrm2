...
(function () {
    ...
    function doFetchInbox(silent) {
        ...
        apiFetch(INBOX_URL, params).then(function (data) {
            ...
            if (!silent && btn) { btn.disabled = false; btn.innerHTML = crmIcon('sync-alt') + ' Get Emails'; }
        });
    }
    ...
    function showNewMailBanner(count) {
        ...
        newMailBanner.innerHTML = crmIcon('envelope') + ' ' + count + ' new email' + (count > 1 ? 's' : '') + ' received — click to view';
    ...
    document.querySelector('.btn-fetch-sent') && document.querySelector('.btn-fetch-sent').addEventListener('click', function () {
        ...
        btn.disabled = true; btn.innerHTML = crmIconSpinner(' Loading...');
        ...
        apiFetch(SENT_URL, params).then(function (data) {
            ...
            btn.disabled = false;
            btn.innerHTML = crmIcon('sync-alt') + ' Get Emails';
        });
    });
    ...
    document.querySelector('.btn-fetch-drafts') && document.querySelector('.btn-fetch-drafts').addEventListener('click', function () {
        ...
        apiFetch(DRAFTS_URL, params).then(function (data) {
          ...
          .finally(function () { btn.disabled = false; btn.innerHTML = crmIcon('sync-alt') + ' Get Drafts'; });
    });
    ...
    (function () {
        ...
        if (saveDraftBtn) {
            saveDraftBtn.addEventListener('click', function () {
                ...
                saveDraftBtn.innerHTML = crmIconSpinner(' Saving…');
                ...
                fetch(SAVE_DRAFT_URL, {
                    ...
                    saveDraftBtn.innerHTML = crmIcon('save') + ' Save Draft';
                });
            });
        }
        ...
        if (form) {
            form.addEventListener('submit', function (ev) {
                ...
                if (sendBtn)  { sendBtn.disabled = true; sendBtn.innerHTML = crmIconSpinner(' Sending…'); }
                ...
                fetch(form.action, {
                    ...
                    if (sendBtn) { sendBtn.disabled = false; sendBtn.innerHTML = crmIcon('paper-plane') + ' Send'; }
                });
            });
        }
    }());
