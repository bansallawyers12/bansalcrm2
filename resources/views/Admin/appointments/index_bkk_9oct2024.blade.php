						...
						<div class="card-body">
							...
							<form action="{{ route('appointments.index') }}" method="get">
								<div class="row">
									...
									<div class="col-md-4">
										<input type="text" class="form-control mb-3" placeholder="Search with user name ...." name="q">
									</div>
									<div class="col-md-2">
								...
								<div class="tab-pane fade show active" id="active_quotation" role="tabpanel" aria-labelledby="active_quotation-tab">
									...
									<table class="table table-bordered">
										...
										<tr>
										    <?php //echo "<pre>client==";print_r($appointment->clients->client_id );die;?>
											<td>{{ ++$i }}</td>
											<!-- @if($appointment->user)
							...
							<td><a href="{{URL::to('/admin/clients/detail/'.base64_encode(convert_uuencode(@$appointment->client_id)))}}" target="_blank" >{{ $appointment->clients->client_id ?? 'N/A' }}</a></td>
											<td>{{ $appointment->clients->first_name ?? 'N/A' }}  {{$appointment->clients->last_name ?? 'N/A'}}</td>
