					...
					<div class="card">
						...
						<div class="card-body">
							<ul class="nav nav-pills" id="checkin_tabs" role="tablist">
								...
								<li class="nav-item">
									<a class="nav-link " id="waiting-tab"  href="{{URL::to('/admin/office-visits/waiting')}}" >Waiting</a>
								</li>
								<li class="nav-item">
									<a class="nav-link" id="attending-tab"  href="{{URL::to('/admin/office-visits/attending')}}" >Attending</a>
								</li>
								<li class="nav-item">
									<a class="nav-link active" id="completed-tab"  href="{{URL::to('/admin/office-visits/completed')}}" >Completed</a>
								</li>
								<li class="nav-item">
									<a class="nav-link" id="archived-tab"  href="{{URL::to('/admin/office-visits/archived')}}" >Archived</a>
								</li>		
								<li class="nav-item">
									<a class="nav-link " id="all-tab"  href="{{URL::to('/admin/office-visits')}}" >All</a>
								</li>
							</ul> 
							...
							<div class="mydropdown" style="margin-top:10px;">
								  ...
								  <a href="{{URL::to('/admin/office-visits/completed')}}">All Branches</a>
								  <?php
								  $branchs = \App\Branch::all();
								  foreach($branchs as $branch){
									?>
									<a href="{{URL::to('/admin/office-visits/completed')}}?office={{$branch->id}}&office_name={{$branch->office_name}}">{{$branch->office_name}}</a>
								  <?php } ?>
										...
										<table class="table text_wrap">
											...
											<tbody class="tdata checindata">	
												...
												<tr did="{{@$list->id}}" id="id_{{@$list->id}}"> 
													<td><a id="{{@$list->id}}" class="opencheckindetail" href="javascript:;">#{{$list->id}}</a></td>
													<td><a href="javascript:;">{{date('l',strtotime($list->created_at))}}</a><br>{{date('Y-m-d',strtotime($list->created_at))}}</td>
													<td><?php if($list->sesion_start != ''){ echo date('h:i A',strtotime($list->sesion_start)); }else{ echo '-'; } ?></td>
													<td><?php if($list->sesion_end != ''){ echo date('h:i A',strtotime($list->sesion_end)); }else{ echo '-'; } ?></td>
