					...
					<div class="card">
						<div class="card-body p-0" style="display: block;">
							<ul class="nav nav-pills flex-column"> <!---->
								<li class="nav-item"> <a href="{{route('returnsetting')}}" id="ember5168" class="nav-link active ember-view"> GST Settings </a> </li> <!----><!----> </ul>
						</div>
					</div>
				</div>
				<div class="col-md-8">
							...
							<div class="row">
								...
								<div class="col-sm-12 <?php if(@Auth::user()->is_business_gst != ''){ if(@Auth::user()->is_business_gst == 'yes'){ ?><?php }else{ ?><?php }}else{ ?>is_gst_yes<?php } ?>" >
									<div class="form-group float-end">
										{!! Form::button(\App\Helpers\IconHelper::render('save') . ' Save', ['class'=>'btn btn-primary', 'onClick'=>'customValidate("add-city")'])  !!}
									</div> 
								</div> 
							</div> 
						</div> 
