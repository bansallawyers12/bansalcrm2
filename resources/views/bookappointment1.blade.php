										...
										<div class="row"> 
											...
											<!-- <div class="col-md-12">
												<div class="form-btn text-right">
													<input type="button" class="btn nextbtn" data-steps="services" name="submit" value="Next" />
												</div>
											</div> -->
										</div>
									</div>
...
jQuery(document).ready(function($){
	...
	$(document).delegate('.enquiry_item', 'click', function(){
		var v = 'services';
	...
	$(document).delegate('.appointment_item', 'click', function(){
		var v = 'info';
		var id = $(this).attr('data-id');
	...
	$(document).delegate('.services_item', 'click', function(){
		...
		if(v == 'datetime'){
			...
			if(!phonenumber(phone)){ 
				...
				if(!$.trim(phone)){
					...
					$('.phone').after('<span class="custom-error" role="alert">Enter a valid phone number.!</span>');  
				}
			}
